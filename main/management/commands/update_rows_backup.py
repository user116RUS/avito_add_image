...
OUTPUT_EXCEL_PATH = "few_cities_local.xlsx"
GOOGLE_CRED_PATH = "google_cred.json"  # Изменено на Service Account credentials
MAX_ITEMS = 1 # Убираем ограничение для продакшена
...
SERVER_BASE_URL = "http://localhost:8000"  # Базовый URL сервера (изменить на продакшн URL)
...
def upload_to_google_drive(file_path, force_update=True):
    ...
    try:
        ...
        # Определяем папку для сохранения (корневая папка)
        folder_id = None
        if GOOGLE_DRIVE_FOLDER_ID:
            ...
            if check_folder_access(drive_service, GOOGLE_DRIVE_FOLDER_ID):
                ...
                print(f"⚠️ Папка с ID {GOOGLE_DRIVE_FOLDER_ID} недоступна, файл будет сохранен в корневой папке")
        ...
        if files:
            file_id = files[0]['id']
            if not force_update:
                print(f"📄 Файл '{file_name}' уже существует на Google Drive")
        ...
        if file_id:
            ...
            print(f"🔄 Обновляем файл '{file_name}' на Google Drive...")
            ...
            # Создаем новый файл
            print(f"📤 Загружаем новый файл '{file_name}' на Google Drive...")
        ...
        if file_id:
            ...
            try:
                ...
                print(f"🔗 Файл '{file_name}' успешно загружен и доступен по ссылке")
            except Exception as e:
                print(f"⚠️ Файл загружен, но не удалось сделать его публичным: {e}")
            
            return f"https://drive.google.com/file/d/{file_id}/view"
        else:
            print(f"❌ Не удалось получить ID загруженного файла")
            return None
...
def authenticate_google_service_account():
    ...
    Возвращает: объект учетных данных или None в случае ошибки
    """
    try:
        if not os.path.exists(GOOGLE_CRED_PATH):
            print(f"❌ Файл учетных данных не найден: {GOOGLE_CRED_PATH}")
            return None
...
def create_drive_service():
    ...
    Возвращает: объект сервиса Google Drive или None в случае ошибки
    """
    try:
        ...
        print(f"❌ Ошибка при создании сервиса Google Drive: {e}")
        return None
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--skip-sync',
            action='store_true',
            help='Пропустить синхронизацию с Google Drive',
        )
        parser.add_argument(
            '--check-consistency',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        if GOOGLE_DRIVE_FOLDER_ID:
            try:
                drive_service = create_drive_service()
                if not drive_service:
                    self.stdout.write(self.style.ERROR("❌ Ошибка инициализации Drive сервиса"))
                    return
                
                if not check_folder_access(drive_service, GOOGLE_DRIVE_FOLDER_ID):
                    self.stdout.write(self.style.WARNING(f"⚠️ Не удалось получить доступ к папке с ID {GOOGLE_DRIVE_FOLDER_ID}"))
                    self.stdout.write("Будет использоваться автоматическое создание папки или корневая папка")
            except Exception as e:
                self.stdout.write(self.style.ERROR(f"❌ Ошибка при проверке доступа к папке Google Drive: {e}"))
        ...
        # Синхронизация с Google Drive (если не пропускаем)
        if not options.get('skip_sync'):
            self.stdout.write("📥 Синхронизация с Google Drive...")
            sync_success = sync_excel_from_gdrive()
            if sync_success:
                self.stdout.write(self.style.SUCCESS("✅ Синхронизация с Google Drive завершена успешно"))
            else:
                self.stdout.write(self.style.WARNING("⚠️ Синхронизация с Google Drive завершена с предупреждениями"))
...
def ensure_django_server_running():
    """Убеждается, что Django сервер запущен"""
