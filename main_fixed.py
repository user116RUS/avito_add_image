...
def set_attributes_for_account(xml_url, local_xml, output_excel, google_cred, images_folder, 
              gdrive_folder_id, shop_images_cache, city_list, description, is_city, max_items=999999999,
              overlay_images=["images/1.png", "images/2.png", "images/3.png", "images/4.png"], watermark_path='images/1.png', 
              shop_images=["shop/photo_1_2025-04-10_16-52-54.jpg", "shop/photo_3_2025-04-10_16-52-54.jpg", "shop/photo_7_2025-04-10_16-52-54.jpg"]):
    '''
        Проставляет все пути и параметры для аккаунта и записывает их в глобальные переменные
    '''
    global XML_URL, LOCAL_XML_PATH, OUTPUT_EXCEL_PATH, GOOGLE_CRED_PATH, IMAGES_FOLDER_NAME, GOOGLE_DRIVE_FOLDER_ID, SHOP_IMAGES_CACHE_FILE, \
    CITY_LIST, NEW_DESCRIPTION, CITY_LIST, OVERLAY_IMAGES, WATERMARK_PATH, SHOP_IMAGES, MAX_ITEMS, IS_CITY

    XML_URL = xml_url
    LOCAL_XML_PATH = local_xml
    OUTPUT_EXCEL_PATH = output_excel
    GOOGLE_CRED_PATH = google_cred
    IMAGES_FOLDER_NAME = images_folder
    GOOGLE_DRIVE_FOLDER_ID = gdrive_folder_id
    SHOP_IMAGES_CACHE_FILE = shop_images_cache
    CITY_LIST = city_list
    ...
    if os.path.exists(output_path):
        ...
        if 'Id' in df.columns and 'Id' in existing_data.columns:
            ...
            if len(new_rows) > 0:
                ...
                temp_output = f"{output_path.split('/')[0]}/{output_path.split('/')[1]}/temp_{output_path.split('/')[2]}"
...
def process_xml(use_gdrive_for_images=True):
    ...
    output_xml_path = f"{"/".join(IMAGES_FOLDER_NAME.split('/')[:2])}/avito_processed.xml"
