...
# Текст для замены в описании
DELIVERY_TEXT_OLD = """<p>🚚<strong>Доставка по РФ</strong> через Авито: Почта России, СДЭК, Boxberry<br /> + Ежедневная отправка<br /> + Надежная упаковка (ничего не повредится)<br /> + Проверка при получении + гарантия</p>
...
DELIVERY_TEXT_NEW_TEMPLATE = """<p>🚚<strong> Доставка в {city}</strong> через Авито: Почта России, СДЭК, Boxberry<br /> + Ежедневная отправка<br /> + Надежная упаковка (ничего не повредится)<br /> + Проверка при получении + гарантия</p>
...
def generate_unique_images(original_url, ad_id, city_index, drive_service):
    ...
    try:
        ...
        # Создаем уникальные варианты для каждого изображения
        for i in range(1, 16):
            ...
            # Генерируем URL модифицированного изображения
            modified_url = cloudinary.CloudinaryImage(upload_result['public_id']).build_url(
...
def create_duplicates(df, drive_service):
    ...
    for _, row in df.iterrows():
        ...
        original_images = str(row['ImageUrls']).split('|') if pd.notna(row['ImageUrls']) else []
        
        # Добавляем исходную строку (помечаем как исходную)
        row_copy = row.copy()
        row_copy['IsOriginal'] = True
        duplicates.append(row_copy)
        
        # Создаем 15 дубликатов для каждого города
        for i, city in enumerate(CITIES, 1):
            dup_row = row.copy()
            
            # Модифицируем ID
            dup_row['Id'] = f"{original_id}-{i}"
            
            # Модифицируем адрес
            dup_row['Address'] = city
            
            # Модифицируем описание
            dup_row['Description'] = modify_description(dup_row['Description'], city)
            
            # Модифицируем изображения (кроме последних - фото магазина)
            if original_images:
                shop_images = original_images[-3:]  # Последние 3 - фото магазина
                product_images = original_images[:-3] if len(original_images) > 3 else original_images
            ...
            dup_row['OriginalId'] = original_id  # Сохраняем связь с оригиналом
...
def sync_with_xml(duplicated_df):
    """
    Синхронизирует данные с XML - удаляет товары, которых нет в XML
    """
    # Загружаем текущий XML
    tree = ET.parse(LOCAL_XML_PATH)
    root = tree.getroot()
    
    # Получаем актуальные ID из XML
    xml_ids = {ad.find('Id').text for ad in root.findall('Ad') if ad.find('Id') is not None}
    
    # Фильтруем DataFrame - оставляем только товары, которые есть в XML
    # Удаляем как оригиналы, так и их дубли
    filtered_df = duplicated_df[
        duplicated_df['OriginalId'].isin(xml_ids) | 
        duplicated_df['Id'].isin(xml_ids)
    ]
...
def process_xml(use_gdrive_for_images=True):
    """Обработка XML-файла и создание Excel-таблицы с дубликатами"""
    ...
    all_parameters.add("IsOriginal")
    all_parameters.add("OriginalId")
    ...
    for ad in root.findall("Ad"):
        ...
        row_data["IsOriginal"] = True
        row_data["OriginalId"] = ad_id
    ...
    # Создаем Excel-файл с цветовым выделением оригиналов
    with pd.ExcelWriter(OUTPUT_EXCEL_PATH, engine='openpyxl') as writer:
        synced_df.to_excel(writer, index=False)
        
        # Получаем доступ к листу для форматирования
        workbook = writer.book
        worksheet = writer.sheets['Sheet1']
        
        # Форматируем строки - желтый для оригиналов
        for idx, row in synced_df.iterrows():
            if row['IsOriginal']:
                for col_num in range(1, len(synced_df.columns) + 1):
                    worksheet.cell(row=idx+2, column=col_num).fill = PatternFill(start_color='FFFF00', 
                                                                               end_color='FFFF00', 
                                                                               fill_type='solid')
    
    # Загружаем файл на Google Drive
    file_url = upload_to_google_drive(OUTPUT_EXCEL_PATH, force_update=True)
    print(f"Таблица с дубликатами загружена на Google Drive")
    
    return synced_df, file_url
...
def job():
    ...
    if download_xml():
        ...
        if not df.empty:
            # Создаем дубликаты и синхронизируем с XML
            duplicated_df = create_duplicates(df)
            synced_df = sync_with_xml(duplicated_df)
            
            # Сохраняем результат с цветовым выделением
            with pd.ExcelWriter(OUTPUT_EXCEL_PATH, engine='openpyxl') as writer:
                synced_df.to_excel(writer, index=False)
                ...
                # Форматирование для оригинальных записей
                yellow_fill = PatternFill(start_color='FFFF00', end_color='FFFF00', fill_type='solid')
                
                for idx, row in enumerate(synced_df.itertuples(), 2):
                    if row.IsOriginal:
                        for col in range(1, len(synced_df.columns)+1):
                            worksheet.cell(row=idx, column=col).fill = yellow_fill
            
            # Загружаем на Google Drive
            file_url = upload_to_google_drive(OUTPUT_EXCEL_PATH)
            print(f"Ссылка на обработанный документ с дубликатами: {file_url}")
        else:
...
def main():
    ...
    required_files = {
        "shop": SHOP_IMAGES,
        "images": OVERLAY_IMAGES + [WATERMARK_PATH],
        "credentials": [GOOGLE_CRED_PATH]
    }
    
    for folder, files in required_files.items():
        if folder in ["shop", "images"]:
            for file in files:
                if not os.path.exists(file):
                    print(f"Внимание: отсутствует файл {file}")
    
    # Проверка Cloudinary конфигурации
    if not all([cloudinary.config().cloud_name, cloudinary.config().api_key, cloudinary.config().api_secret]):
        print("Внимание: Cloudinary не настроен правильно!")
    
    # Первый запуск
    job()
    
    # Настраиваем регулярное выполнение (каждые 5 минут)
